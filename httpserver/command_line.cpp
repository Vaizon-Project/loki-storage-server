...
void command_line_parser::parse_args(int argc, char* argv[]) {
    ...
    // clang-format off
    desc_.add_options()
        ("data-dir", po::value(&options_.data_dir), "Path to persistent data (defaults to ~/.vaizon/storage)")
        ...
        ("vaizond-rpc-ip", po::value(&options_.vaizond_rpc_port), "RPC IP on which the local Vaizon daemon is listening (usually localhost)")
        ("vaizond-rpc-port", po::value(&options_.vaizond_rpc_port), "RPC port on which the local Vaizon daemon is listening")
    ...
    hidden.add_options()
        ...
        ("vaizond-key", po::value(&options_.vaizond_key), "Legacy secret key (test only)")
        ("vaizond-x25519-key", po::value(&options_.vaizond_x25519_key), "x25519 secret key (test only)")
        ("vaizond-ed25519-key", po::value(&options_.vaizond_ed25519_key), "ed25519 public key (test only)");
    // clang-format on
    ...
    if (options_.testnet && !vm.count("vaizond-rpc-port")) {
        options_.vaizond_rpc_port = 38157;
    }
