...
RequestHandler::RequestHandler(boost::asio::io_context& ioc, ServiceNode& sn,
                               const VaizondClient& vaizond_client,
                               const ChannelEncryption<std::string>& ce)
    : ioc_(ioc), service_node_(sn), vaizond_client_(vaizond_client),
      channel_cipher_(ce) {}
...
Response RequestHandler::handle_wrong_swarm(const user_pubkey_t& pubKey) {
    ...
    VAIZON_LOG(trace, "Got client request to a wrong swarm");
...
Response RequestHandler::process_store(const json& params) {
    ...
    for (const auto& field : fields) {
        if (!params.contains(field)) {
            ...
            VAIZON_LOG(debug, "Bad client request: no `{}` field", field);
    ...
    VAIZON_LOG(trace, "Storing message: {}", data);
    ...
    if (data.size() > MAX_MESSAGE_BODY) {
        VAIZON_LOG(debug, "Message body too long: {}", data.size());
...
#ifndef DISABLE_POW
    if (!valid_pow) {
        VAIZON_LOG(debug, "Forbidden. Invalid PoW nonce: {}", nonce);
    ...
    if (!success) {
        ...
        VAIZON_LOG(warn, "Service node is initializing");
        return Response{Status::SERVICE_UNAVAILABLE, "Service node is initializing\n"};
    }

    VAIZON_LOG(trace, "Successfully stored message for {}",
             obfuscate_pubkey(pk.str()));
...
Response RequestHandler::process_snodes_by_pk(const json& params) const {
    ...
    if (!params.contains("pubKey")) {
        VAIZON_LOG(debug, "Bad client request: no `pubKey` field");
    ...
    VAIZON_LOG(debug, "Snodes by pk size: {}", nodes.size());

    const json res_body = snodes_to_json(nodes);

    VAIZON_LOG(debug, "Snodes by pk: {}", res_body.dump());
...
Response RequestHandler::process_retrieve(const json& params) {
    ...
    if (!service_node_.retrieve(pk.str(), last_hash, items)) {
        ...
        auto msg = fmt::format(
            ...
            obfuscate_pubkey(pk.str()));
        VAIZON_LOG(critical, "{}", msg);

        return Response{Status::INTERNAL_SERVER_ERROR, std::move(msg)};
    }

    if (!items.empty()) {
        VAIZON_LOG(trace, "Successfully retrieved messages for {}",
                 obfuscate_pubkey(pk.str()));
    }
...
void RequestHandler::process_client_req(const std::string& req_json, std::function<void(vaizon::Response)> cb) {
    ...
    if (method_name == "store") {
        ...
        VAIZON_LOG(debug, "Process client request: snodes for pubkey");
        ...
        VAIZON_LOG(debug, "Bad client request: unknown method '{}'", method_name);
...
void RequestHandler::process_lns_request(
    lokimq::string_view name_hash, std::function<void(vaizon::Response)> cb) {
    ...
    auto on_vaizond_res = [cb = std::move(cb)](sn_response_t sn) {
        if (sn.error_code == SNodeError::NO_ERROR && sn.body) {
            cb({Status::OK, *sn.body});
        } else {
            cb({Status::BAD_REQUEST, "unknown vaizond error"});
        }
    };

#ifdef INTEGRATION_TEST
    // use mainnet seed
    vaizond_client_.make_custom_vaizond_request("public.vaizon.foundation", 22023,
                                                "lns_names_to_owners", params,
                                                std::move(on_vaizond_res));
#else
    vaizond_client_.make_vaizond_request("lns_names_to_owners", params, std::move(on_vaizond_res));
#endif
...
RequestHandler::process_onion_exit(const std::string& eph_key,
                                   ...
                                   std::function<void(vaizon::Response)> cb) {

    VAIZON_LOG(debug, "Processing onion exit!");
    ...
    try {
        ...
        VAIZON_LOG(debug, "CLIENT HEADERS: \n\t{}", req.at("headers").dump(2));
    } catch (std::exception& e) {
...
void RequestHandler::process_proxy_exit(
    ...
    VAIZON_LOG(debug, "[{}] Process proxy exit", idx);
    ...
    try {
        ...
        if (req.find("headers") != req.end()) {
            if (req.at("headers").find(VAIZON_LONG_POLL_HEADER) != req.at("headers").end()) {
                lp_used = req.at("headers").at(VAIZON_LONG_POLL_HEADER).get<bool>();
            }
        }

    } catch (std::exception& e) {
        auto msg = fmt::format("JSON parsing error: {}", e.what());
        VAIZON_LOG(debug, "[{}] {}", idx, msg);
    ...
    if (lp_used) {
        VAIZON_LOG(debug, "Long polling requested over a proxy request");
    }

    this->process_client_req(body, [this, cb = std::move(cb), client_key,
                                    idx](vaizon::Response res) {
        VAIZON_LOG(debug, "[{}] proxy about to respond with: {}", idx,
                 res.status());
...
Response RequestHandler::process_onion_to_url(
    ...
    const std::string& payload, std::function<void(vaizon::Response)> cb) {
    ...
    auto http_cb = [cb = std::move(cb)](sn_response_t res) {
        ...
        if (res.error_code == SNodeError::NO_ERROR) {
            cb(vaizon::Response{Status::OK, *res.body});
        } else {
            VAIZON_LOG(debug, "Vaizon server error: {}", res.error_code);
            cb(vaizon::Response{Status::BAD_REQUEST, "Vaizon Server error"});
        }
    };
...
void RequestHandler::process_onion_req(const std::string& ciphertext,
    ...
    if (!service_node_.snode_ready(boost::none)) {
        cb(vaizon::Response{Status::SERVICE_UNAVAILABLE, "Snode not ready"});
        return;
    }
    ...
    try {
        ...
        VAIZON_LOG(debug, "Error decrypting an onion request: {}", e.what());
        // Should this error be propagated back to the client? (No, if we
        // couldn't decrypt, we probably won't be able to encrypt either.)
        cb(vaizon::Response{Status::BAD_REQUEST, "Invalid ciphertext"});
        return;
    }

    VAIZON_LOG(trace, "onion request decrypted: <{}>", plaintext);
    ...
    try {
        ...
        if (inner_json.find("body") != inner_json.end()) {
            ...
            this->process_onion_exit(
                ephem_key, plaintext,
                [this, ephem_key, cb = std::move(cb)](vaizon::Response res) {
            ...
            VAIZON_LOG(trace, "We are to forward the request to url: {}{}", host, target);
            ...
            if ((target.rfind("/lsrpc") == target.size() - 6) && (target.find('?') == std::string::npos)) {
                ...
                auto res = vaizon::Response{Status::BAD_REQUEST, "Invalid url"};
        ...
        if (!sn) {
            ...
            VAIZON_LOG(warn, "{}", msg);
            auto res = vaizon::Response{Status::BAD_GATEWAY, std::move(msg)};
            auto wrapped_res = this->wrap_proxy_response(res, ephem_key, true);
            cb(std::move(res));
            return;
        }
        ...
        auto on_response = [cb, counter_copy = counter](bool success, std::vector<std::string> data) {
            ...
            if (!success) {
                VAIZON_LOG(debug, "[Onion request] Request time out");
                cb(vaizon::Response{Status::GATEWAY_TIMEOUT, "Request time out"});
                return;
            }

            // We only expect a two-part message
            if (data.size() != 2) {
                VAIZON_LOG(debug, "[Onion request] Incorrect number of messages: {}", data.size());
                cb(vaizon::Response{Status::BAD_REQUEST, "Incorrect number of messages"});
                return;
            }

            /// We use http status codes (for now)
            if (data[0] == "200") {
                cb(vaizon::Response{Status::OK, std::move(data[1])});
            } else {
                VAIZON_LOG(debug, "Onion request relay failed with: {}", data[1]);
                // When do we ever get this??
                cb(vaizon::Response{Status::SERVICE_UNAVAILABLE, ""});
            }

        };

        VAIZON_LOG(debug, "send_onion_to_sn, sn: {} reqidx: {}", *sn, counter++);

        service_node_.send_onion_to_sn(*sn, payload, ekey, on_response);

    } catch (std::exception& e) {
        VAIZON_LOG(debug, "Error parsing inner JSON in onion request: {}", e.what());

        auto res = vaizon::Response{Status::BAD_REQUEST, "Invalid json"};
