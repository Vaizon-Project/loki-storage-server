...
void serialize_message(std::string& res, const T& msg) {
    ...
    VAIZON_LOG(trace, "serialized message: {}", msg.data);
}
...
std::vector<message_t> deserialize_messages(const std::string& blob) {
    ...
    VAIZON_LOG(trace, "=== Deserializing ===");
    ...
    while (!slice.empty()) {
        ...
        /// Deserialize PK
        auto pk = deserialize_string(slice, vaizon::get_user_pubkey_size());
        if (!pk) {
            VAIZON_LOG(debug, "Could not deserialize pk");
            return {};
        }
        ...
        if (!hash) {
            VAIZON_LOG(debug, "Could not deserialize hash");
            return {};
        }
        ...
        if (!data) {
            VAIZON_LOG(debug, "Could not deserialize data");
            return {};
        }
        ...
        if (!ttl) {
            VAIZON_LOG(debug, "Could not deserialize ttl");
            return {};
        }
        ...
        if (!timestamp) {
            VAIZON_LOG(debug, "Could not deserialize timestamp");
            return {};
        }
        ...
        if (!nonce) {
            VAIZON_LOG(debug, "Could not deserialize nonce");
            return {};
        }

        VAIZON_LOG(trace, "Deserialized data: {}", *data);

        VAIZON_LOG(trace, "pk: {}, msg: {}", *pk, *data);

        result.push_back({*pk, *data, *hash, *ttl, *timestamp, *nonce});
    }

    VAIZON_LOG(trace, "=== END ===");
